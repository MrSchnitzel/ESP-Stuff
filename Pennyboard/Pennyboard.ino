...
RGB leds[LED_COUNT];
QMC5883L compass;
float compassValuePrev;
Preferences preferences;
	uint8_t pressed = digitalRead(MODE_BUTTON);
...
//		printMode();
	...
	if (pressed != 0){
		current_mode +=1;
		current_mode = current_mode < 6 ? current_mode : 0;
	}
	switch (current_mode) {
...
void blinkRGB(int speed) {
	...
	uint8_t pressed = pressed || digitalRead(MODE_BUTTON);
	delay(speed);
...
void showCompass() {
	...
	uint8_t pressed = pressed || digitalRead(MODE_BUTTON);
	if ((compassValuePrev - threshold) < heading
			&& heading < (compassValuePrev + threshold)) {
		return;
	}
	compassValuePrev = heading;
	float percent = 100 * heading / 360;

	RGB farben1[anzahl];
	RGB farben2[anzahl];

	getColorArray(mainCol, RGB(255, 255, 255), anzahl, farben1);
	getColorArray(RGB(255, 255, 255), mainCol, anzahl, farben2);

	int pos = band1.getPositionFromPercent(percent);

	band1.setAllLed(mainCol);
//	Serial.println(pos);
	band1.setLEDArray(pos - anzahl, anzahl, farben1, 0, true);
	band1.setLED(pos, RGB(0, 255, 0), 0, true);
	band1.setLEDArray(pos + 1, anzahl, farben2, 0, true);
	band1.saveSet();
	delay(50);
}
